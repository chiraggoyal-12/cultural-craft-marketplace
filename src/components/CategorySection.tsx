...
const CategorySection = () => {
  ...
  return (
    <section className="py-20 sm:py-24 bg-gradient-warm">
      <div className="container mx-auto px-4">
        <div className="text-center mb-8 sm:mb-12">
          ...
          <p className="text-base sm:text-lg text-muted-foreground max-w-2xl mx-auto px-4">
            Explore our curated collections of authentic handcrafted items, each
            telling a unique story of tradition and artistry.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
          {categories.map((category) => (
            <Card
              key={category.title}
              className="group overflow-hidden border-0 shadow-cultural hover:shadow-elegant smooth-transition hover:scale-105 bg-card/50 backdrop-blur-sm"
            >
              <div className="relative overflow-hidden">
                <img
                  src={category.image}
                  alt={category.title}
                  className="w-full h-48 sm:h-56 md:h-64 object-cover smooth-transform group-hover:scale-110"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-warm-brown/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 smooth-transition" />
              </div>

              <CardContent className="p-4 sm:p-6">
                <h3 className="text-lg sm:text-xl font-semibold text-foreground mb-2 group-hover:text-primary smooth-transition">
                  {category.title}
                </h3>
                ...
                <Button
                  variant="ghost"
                  className="w-full justify-start p-0 h-auto text-primary hover:text-primary/80 font-medium group-hover:translate-x-1 smooth-transform"
                >
                  Explore Collection →
                </Button>
              </CardContent>
